...
## 검증 결과 — 실제 화면 (2026-09-19, Pixel9_arm 에뮬레이터, 한국어 로캘, 게스트)
- 한국어 홈 첫 카드 "오늘 연습한 대사, 녹음하고 피드백 받기"와 바로 아래 2인 대사 연습 카드: 확인.
- 앱 강제 종료 후 재실행 시 가입 화면 없이 홈 직행(`artlink-guest-entered` 저장): 확인.
- 2인 대사 대본 모드 하단 "연습 기록 남기기"·"연습 끝": 확인. 누르면 제목·안내 문구·연기 분야·시리즈가 채워진 노트 작성 화면: 확인.
- 프로덕션 서버로 AI 피드백 수신 후 후보 칩 3개 표시, 선택 표시, 저장 후 상세에서 선택 유지: 확인.
- "이 포인트로 다시 연습" → 새 노트 상단 "이번 연습의 초점" 고정: 확인. 재연습 노트 AI 탭의 "지난 연습" 카드(지난 초점 표시): 확인.
- 계측: 이 기기로 practice_events 7건 적재 확인 후 테스트 행 전부 삭제.
- 미확인: 큐 연습 모드 마지막 줄 버튼, 재연습 노트의 AI 비교 단락(게스트 하루 한도로 2회차 호출 안 함), iOS 실기기, 녹음·영상 첨부 경로(에뮬레이터 마이크 불가).
- 관찰: 홈에 2인 대사 진입이 카드와 퀵 노트 하단 두 곳에 중복 노출. AI 데이터 안내 문구가 아직 "OpenAI"로 표기(기존 후속 항목).
