...
APP_ID = "6752890224"
VERSION = "1.10.19"
BUILD_NUMBER = nil  # 1.10.19 최신 빌드 자동 선택
NOTES_PATH = "/private/tmp/claude-501/-Users-leechangyeop/b53e6f2f-d257-4167-bb2d-87e12b7ac093/scratchpad/release-notes-1.10.19.json"
...
build_id = nil
60.times do |i|
  c, r = asc(:get, "builds?filter[app]=#{APP_ID}&filter[preReleaseVersion.version]=#{VERSION}&sort=-uploadedDate&limit=1")
  b = (c == 200) ? r["data"]&.first : nil
  st = b && b["attributes"]["processingState"]
  if st == "VALID"
    build_id = b["id"]
    puts "STEP2: 빌드 #{b["attributes"]["version"]} VALID (#{build_id})"
    break
...
puts(state == "WAITING_FOR_REVIEW" ? "=== SUCCESS: 1.10.18 심사 제출 완료 ===" : "=== 확인 필요: #{state} ===")
